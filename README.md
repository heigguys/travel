...
后端已关闭启动时的自动 SQL 初始化，重启服务不会执行初始化脚本，也不会清空咨询消息或其他业务数据。

首次创建空数据库时，需要手动按以下顺序执行：

1. `back-end/src/main/resources/schema.sql`
2. `back-end/src/main/resources/data.sql`

`schema.sql` 会删除并重建现有数据表，只能用于首次初始化空数据库。已有业务数据的数据库禁止重复执行该脚本。`data.sql` 用于写入默认用户、旅行计划和申请数据。
